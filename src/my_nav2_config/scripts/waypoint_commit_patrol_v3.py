...
class WaypointCommintPatrol(Node):
	...
	self.goal_id_pub = self.create_publisher(Int32, 'robot_status/reached_goal_id',10)
        # ── 订阅器（来自上位机 WiFi 话题）──────────────────────────────────
	...
	self.current_goal_id = self.current_wp_index + 1
	id_msg = Int32()
	id_msg.data = self.current_goal_id
	self.goal_id_pub.publish(id_msg)
