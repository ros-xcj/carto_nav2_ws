...
from std_msgs.msg import UInt8, UInt32,Int32
...
class WaypointCommintPatrol(Node):
    ...
    def __init__(self):
        ...
        # ── ROS2 参数声明 ────────────────────────────────────────────────────
        self.declare_parameter('dwell_time',                1.5)   # 每点停留秒数
        self.declare_parameter('max_loops',                 0)      # 最大循环次数, 0=无限, 1=单次
        self.declare_parameter('skip_on_failure',           True)   # 导航失败时跳过
        self.declare_parameter('min_interaction_duration',  4.0)    # 最短中断保持时间(s)
        self.declare_parameter('xy_tolerance',              0.1)    # 到达判定半径(m)
        self.declare_parameter('yaw_tolerance',             0.1)    # 角度到达判定(rad)
        ...
        # 声源角方向约定:
        #   True  = 顺时针为正 (0°=前, 90°=右, 270°=左)  ← 大多数麦克风阵列
        #   False = 逆时针为正 (0°=前, 90°=左, 270°=右)  ← 标准数学约定
        self.declare_parameter('angle_clockwise',           True)
        ...
        self.angle_clockwise          = self.get_parameter('angle_clockwise').value
        ...
        # 路点到达通知：每当机器人到达一个路点时，发布其1-based序号 (1, 2, 3, ...)
        # 话题: patrol/current_waypoint  类型: std_msgs/UInt8
        # ID值由 current_wp_index+1 自动计算，增删 waypoints_data 无需修改此处
        self.wp_id_pub = self.create_publisher(Int32, 'robot_status/reached_goal_id', 10)
    ...
    def _handle_navigating(self):
        ...
        if cx is not None:
            gx, gy, gyaw = self.waypoints_data[self.current_wp_index]
            dist = math.hypot(gx - cx, gy - cy)
            yaw_err = gyaw - current_yaw
            yaw_err = math.atan2(math.sin(yaw_err), math.cos(yaw_err))

            if dist <= self.xy_tolerance and abs(yaw_err) <= self.yaw_tolerance:
                self.get_logger().info(
                    f'WP-{self.current_wp_index + 1} ✅ 距离与位姿触发到达 '
                    f'(dist={dist:.2f}m ≤ {self.xy_tolerance}m, '
                    f'yaw_err={math.degrees(abs(yaw_err)):.1f}° ≤ {math.degrees(self.yaw_tolerance):.1f}°)')
    ...
    def _handle_reached_waiting(self):
        ...
        # ★ 持续发布到达的路点ID (10Hz)，防止 echo 时错过单次触发消息
        wp_id_msg = Int32()
        wp_id_msg.data = self.current_wp_index + 1
        self.wp_id_pub.publish(wp_id_msg)
    ...
    def _on_waypoint_reached(self):
        """到达路点：发布路点ID，开始停留计时，切换状态"""
        # 发布路点到达通知（1-based ID，与 waypoints_data 列表下标对应）
        wp_id_msg = Int32()
        wp_id_msg.data = self.current_wp_index + 1
        self.wp_id_pub.publish(wp_id_msg)
        ...
        self.get_logger().info(
            f'WP-{self.current_wp_index + 1}/{self.total_wps} 到达 '
            f'→ 已发布 patrol/current_waypoint={wp_id_msg.data}，'
            f'开始停留 {self.dwell_time}s')
    ...
    def _rotate_in_place(self, angle_deg: float):
        """
        声源定位角 → 机器人原地转向目标 yaw。
        实际旋转由 _handle_interacting 10Hz 循环经 /cmd_vel 持续执行。

        声源角约定（与雷达 +X 轴对齐，机器人本体系）:
          0°   = 正前方
          90°  = 右侧 (angle_clockwise=True) 或 左侧 (angle_clockwise=False)
          180° = 正后方
          270° = 左侧 (angle_clockwise=True) 或 右侧 (angle_clockwise=False)

        ROS2 yaw 约定: 逆时针(CCW)为正。
        angle_clockwise=True  → relative_yaw = -angle_rad
        angle_clockwise=False → relative_yaw = +angle_rad
        使用 atan2 归一化到 [-π, π]，避免跨越 ±π 边界的歧义。
        """
        cx, cy, current_yaw = self._get_robot_pose_full()
        if cx is None:
        ...
        # 将声源角转为 ROS yaw 相对增量，归一化到 [-π, π]
        sign = -1.0 if self.angle_clockwise else 1.0
        angle_rad = math.radians(angle_deg)
        relative_yaw = math.atan2(
            math.sin(sign * angle_rad),
            math.cos(sign * angle_rad)
        )

        self.rotation_target_yaw = current_yaw + relative_yaw
        self.get_logger().info(
            f'[RotateInPlace] 声源角={angle_deg:.1f}° '
            f'方向={"CW" if self.angle_clockwise else "CCW"} '
            f'当前yaw={math.degrees(current_yaw):.1f}° '
            f'相对转角={math.degrees(relative_yaw):.1f}° '
            f'目标yaw={math.degrees(self.rotation_target_yaw):.1f}°')
